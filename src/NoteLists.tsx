...
export function NoteLists({ availableTags, notes, onUpdateTag, onDeleteTag }: NoteListProps) {
    ...
    const filteredNotes = useMemo(() => {
        return notes.filter(note => {
            ...
            // This functions does is loops through all the selectedTags and makes sure evey single one returns true for the statement below
            return (title === "" || note.title.toLowerCase().includes(title.toLowerCase()))

                //  and this line of codes checks the notes to see if it containts the tags we're looping through 
                && (selectedTags.length === 0 || selectedTags.every(tag => note.tags.some(noteTag => noteTag.id === note.id)))
        })
            ...
            <Stack gap={2} className="align-items-center justify-content-center h-100">
                ...
                {tags.length > 0 && (
                    <Stack gap={1} direction="horizontal" className="justify-content-center flex-wrap">
                        {tags.map(tag => (
                            <Badge className="text-turncate" key={tag.id}>{tag.label}</Badge>
                        ))}
